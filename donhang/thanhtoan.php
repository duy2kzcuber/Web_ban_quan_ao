    <?php
    session_start();
    require_once('../admin/ketnoi.php'); // Đảm bảo file kết nối đến cơ sở dữ liệu
    ...
    function calculateTotalAmount() {
        ...
        if (!empty($_SESSION['cart'])) {
            foreach ($_SESSION['cart'] as $product) {
                $totalAmount += ($product['gia'] * $product['soluong'])+$shippingFee;
            }
        }
        return $totalAmount;
    }
    ...
    if (isset($_SESSION['matk'])) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (!empty($_SESSION['cart'])) {
                foreach ($_SESSION['cart'] as $product) {
                    ...
                    $query_product = "SELECT p.masp, p.tensp, ts.tensize, p.gia 
                                    FROM Product p
                                    LEFT JOIN tensize ts ON p.masp = ts.id
                    ...
                    if ($result_product && mysqli_num_rows($result_product) > 0) {
                        ...
                        $masp = $row['masp'];
                        $tensp = $row['tensp'];
                        $tensize = $row['tensize']; // Lấy tensize từ kết quả truy vấn
                        $dongia = $row['gia'];
                    } else {
                        die('Error: Không tìm thấy thông tin sản phẩm.');
                    }

                    $thanhtien = ($sl * $dongia) + $shippingFee;

                    // Insert vào bảng OrderDetails với thông tin tensize
                    $query_order_detail = "INSERT INTO OrderDetails (order_id, masp, tensp, tensize, sl, dongia, thanhtien)
                                        VALUES ('$order_id', '$masp', '$tensp', '$tensize', '$sl', '$dongia', '$thanhtien')";
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $pttt= $_POST['pttt'];
            $shippingFee=50000;
            $totalAmount = calculateTotalAmount($_SESSION['cart'])+$shippingFee;
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        if (!empty($_SESSION['cart'])) {
                                            ...
                                            echo number_format($totalAmount+$shippingFee) . 'đ';
                                        } else {
                                            echo '0đ';
                                        }
                                        ?>
